...
# The funnel counts must agree with the catalogue actually loaded, not with a number
# typed in here. Hardcoding it made four tests fail for the same uninteresting reason
# the first time the catalogue changed size.
...
def test_health_returns_200(client):
    ...
    assert body["catalogue_products"] == CATALOGUE_SIZE
    assert "ml" in body
...
def test_eligibility_returns_one_result_per_product(client):
    ...
    assert resp.status_code == 200
    results = resp.json()
    assert len(results) == CATALOGUE_SIZE
...
def test_recommend_returns_complete_recommendation(client, isolated_store):
    ...
    assert body["coverage"]["catalogue_products"] == CATALOGUE_SIZE
...
def test_scenario_reruns_pipeline_with_modified_inputs(client, isolated_store):
    ...
    # Proving a real re-run, not arithmetic: the scenario's candidate was rebuilt from
    # the modified income, so its EMI and the income in its trace both differ from the
    # baseline's. NOT asserted on ml_suitability: the calibrator is isotonic, so it is
    # a step function, and two genuinely different raw scores can legitimately land on
    # the same plateau. That made this test fail for a reason it was not testing.
    assert scenario["selected_candidate"]["emi"] != baseline["selected_candidate"]["emi"]
    assert baseline["decision_trace"]["financial_metrics"]["monthly_income"] == 120_000.0
...
def test_coverage_returns_funnel(client, isolated_store):
    ...
    assert coverage["catalogue_products"] == CATALOGUE_SIZE
