...
from app.storage import mongo_log
from app.india.rate_cache import get_rates
...
def recommend_endpoint(req: RecommendRequest, background_tasks: BackgroundTasks) -> Recommendation:
    result = recommend(
        req.customer,
        req.portfolio,
        req.requirement,
    ...
    # Fire-and-forget: log session to MongoDB without blocking the response.
    background_tasks.add_task(mongo_log.log_session, req, result)
    return result
...
# ---------------------------------------------------------------------------
# India bank rates — display-only informational endpoint
# ---------------------------------------------------------------------------


@router.get("/india/bank-rates")
async def india_bank_rates() -> dict:
    """
    Returns current indicative Indian bank loan rates.

    DISPLAY-ONLY: These rates are for informational context only. They are not
    used by the recommendation engine or EMI calculations. Rates are sourced from
    publicly available RBI and bank rate-card pages, cached for 24 hours.
    """
    rates = await get_rates()
    return {
        "disclaimer": (
            "Rates shown are indicative, sourced from publicly available RBI and bank "
            "rate cards. Actual offered rates may differ based on your credit profile, "
            "loan amount, and bank policy. These rates do not affect the LOANR recommendation."
        ),
