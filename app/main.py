...
LIFESPAN:
  ...
  - closes the optional MongoDB session logger on shutdown (no-op when unconfigured).
...
async def lifespan(_app: FastAPI):
    ...
    store.close()
    # MongoDB session logger connects lazily on first write; closing is a no-op
    # when it was never configured or never reached.
    await mongo_log.close()
...
app = FastAPI(title="LOANR — Personalized Loan Recommendation", lifespan=lifespan)
